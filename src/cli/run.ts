...
export async function run(argv: string[], deps: CliDeps = defaultDeps): Promise<number> {
  ...
  // A no-subcommand invocation (`luftqualitaet`, `luftqualitaet --compact`) should
  // print help to stdout and exit 0 — matching `--help` — rather than commander's
  // default of help-to-stderr + exit 1, which contradicts the documented
  // "help → exit 0" model and is a spurious failure for scripts building a
  // possibly-empty command. Detect it by parsing only the options (which correctly
  // consumes global-option values) on a throwaway program: empty `operands` with
  // nothing left in `unknown` means no command and no help/version or unknown
  // option. `--help`/`--version`/unknown options land in `unknown` and fall through
  // to commander so it handles (and reports) them exactly as before.
  try {
    // The probe program must be exit-overridden AND silenced: parseOptions runs
    // the option value-parsers (e.g. parseIntArg), so an invalid value would
